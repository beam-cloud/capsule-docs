...
Capsule includes a built-in task execution model, so you do not need to introduce a second worker stack just to keep the app responsive.

Tasks let you run work outside the main chat turn. Schedules let you trigger work on a cron cadence. Together they turn background execution into part of the product surface instead of something bolted on later.
