...
`capsule serve app:app` starts the local dev loop. `capsule deploy app:app` ships the app. In hosted mode, Capsule runs the app in isolated per-user runtimes so each app user has their own sandboxed execution context.
