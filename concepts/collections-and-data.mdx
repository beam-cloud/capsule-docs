...
description: "When to use collections, data handlers, settings, or `session.data`."
---

Capsule gives you several ways to surface and store data, and the names can blur together on a first read.

The easiest way to keep them straight is to ask one question: is this durable app state, computed view data, configuration, or temporary conversational state? Each Capsule primitive maps pretty cleanly to one of those jobs.
...
Do not treat it like your primary record store.
