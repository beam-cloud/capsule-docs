...
When people ask what a Capsule app actually is, the shortest answer is: it is one Python definition that Capsule can run for real users.
...
Capsule runs the app inside a managed runtime per app user. You do not have to invent your own execution layer for sessions, channel traffic, tasks, or page-backed interactions.

That same runtime model powers local iteration with `capsule serve`, hosted deploys with `capsule deploy`, and the channel-facing versions of the app. The main thing that changes is where the requests come from, not what kind of app you are writing.
